...
namespace Programmable_block_test
{
...
//------------------------------------------------------------------------------//
        ...
        public void Main(string argument)
        {
            ...
            IMyTextPanel LCDPanel1 = (IMyTextPanel)GridTerminalSystem.GetBlockWithName("LCD Panel");
            ...
            {
                SolarPower += SolarPanels[i].CurrentOutput;
            }
            
            for (int i = 0; i < Batteries.Count; i++)
            {
                TotalPower += Batteries[i].MaxStoredPower;
                CurrentPower += Batteries[i].CurrentStoredPower;
            }

            float Powerpercentage = (CurrentPower / TotalPower) * 100;            
            
            LCDPanel1.WritePublicText($"Number of Batteries: {Batteries.Count} \n Remaining power: {string.Format("{0:0.00}", Powerpercentage), 2}% \n Solar panel power: {SolarPower} \n Solar panels: {SolarPanels.Count}", false);            
            LCDPanel1.ShowPublicTextOnScreen();
